...
void DMAExample()
{
    if (!mem.Init("wesnoth.exe", false, false))
    {
    ...
    uintptr_t base = mem.GetBaseDaddy("wesnoth.exe");

    int value = 0;
    if (mem.Read(0x17EED18, &value, sizeof(value), mem.GetPidFromName("wesnoth.exe")))
    ...
    if (mem.Read(value + 0xa90, &value, sizeof(value)))
        std::cout << "Read Value" << std::endl;
    else
        std::cout << "Failed to read Value" << std::endl;
    std::cout << "Value: " << value << std::endl;
    ...
    if (mem.Read(value + 4, &value, sizeof(value)))
        std::cout << "Read Value" << std::endl;
    else
        std::cout << "Failed to read Value" << std::endl;
    std::cout << "Value: " << value << std::endl;
    ...
    mem.Write((uintptr_t)(void*)ptr + 4, value+500);
    return;
