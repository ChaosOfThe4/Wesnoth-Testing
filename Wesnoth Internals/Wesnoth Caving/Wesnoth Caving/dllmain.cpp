...
__declspec(naked) void codecave() {
	...
	player_base = (DWORD*)0x017EED18; //player base in wesnoth
	game_base = (DWORD*)(*player_base + 0xA90); // game base in wesnoth
	gold = (DWORD*)(*game_base + 4); //gold location wesnoth
	*gold = 999; //changes gold

	//assembly block to pop everything back off the stack and run the original commands and return to where we placed our jmp for our code cave
	__asm {
		popad
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
	...
	unsigned char* hook_location = (unsigned char*)0x00CCAF8A; //Unsigned char* so we can adjust the individual bytes
